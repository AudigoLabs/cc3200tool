...
class CC3200Connection(object):
    ...
    def _fs_programming(self, flags, chunk, key=''):
        ...
        self._send_packet(command + key + chunk, timeout=100)
    ...
    def switch_uart_to_apps(self):
        ...
        if not self._try_breaking(5, 1, 1.5):
    ...
    def write_file(self, local_file, cc_filename, sign_file=None, size=0):
        ...
        if alloc_size > 200000:
            timeout = max(timeout,
                          5 * ((alloc_size / 200000) + 1))  # empirical value is ~252925 bytes for 5 sec timeout
...
def main():
    ...
    cc.exec_from_ram()  # step 7, initialization is completed.
